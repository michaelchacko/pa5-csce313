...
public:
    ...
    void update(uint personNumber, double dataPoint){
        /*
        if(personNumber > hists.size()){
            cerr << "HistogramCollection.update(): person number outside range of Histogram* vector hists\n";
        }
        */

        hists.at(personNumber-1)->update(dataPoint);
    }
